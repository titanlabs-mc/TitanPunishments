...
public class IpInfoPlayerAccountsSub extends SubCommand<CommandSender> {
    private final UserCache userCache;
    private final Lang lang;

    public IpInfoPlayerAccountsSub(PunishmentPlugin plugin, String permission, boolean isConsole) {
        super(plugin, permission, isConsole);
        this.userCache = plugin.getUserCache();
        this.lang = plugin.getLang();

        this.addArgument(OfflinePlayer.class, "player");
        this.addFlat("accounts");
    }

    @Override
    public void onExecute(CommandSender sender, String[] args) {
        OfflinePlayer offlineTargetPlayer = this.parseArgument(args, 0);
        this.userCache.get(offlineTargetPlayer.getUniqueId()).thenAccept(optionalTarget -> {
